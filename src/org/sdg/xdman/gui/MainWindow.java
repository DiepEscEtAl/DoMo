...
import diep.esc.dev.ExportUtil;
...
public class MainWindow extends JFrame implements ActionListener,
	...
	void createWindow() {
		setTitle("Xtreme Download Manager");
	...
	public void actionPerformed(ActionEvent e) {
		String str = e.getActionCommand();
                ...
                //<diepesc>
                if(str.equals("Export selected items")&&table.getSelectedRows().length<=0){
                    JOptionPane.showMessageDialog(null,"No item selected");
                    return;
                }
                
                if(str.equals("Export all")||str.equals("Export selected items")){
                    JFileChooser fc=new JFileChooser();
                    fc.setApproveButtonText("Save");
                    FileFilter fffAll=fc.getChoosableFileFilters()[0];
                    fc.removeChoosableFileFilter(fffAll);
                    FileFilter fffDME=new FileFilter() {

                        @Override
                        public boolean accept(File f) {
                            return (f.getName().toLowerCase().endsWith(".dme")||!f.isFile())&&!f.isHidden();
                        }

                        @Override
                        public String getDescription() {
                            return "DM Export file format (*.dme)";
                        }
                    };
                    FileFilter fffTXT=new FileFilter() {

                        @Override
                        public boolean accept(File f) {
                            return (f.getName().toLowerCase().endsWith(".txt")||!f.isFile())&&!f.isHidden();
                        }
                    ...
                    fc.addChoosableFileFilter(fffDME);
                    fc.addChoosableFileFilter(fffTXT);
                    fc.addChoosableFileFilter(fffAll);
                    
                    int op=fc.showSaveDialog(this);
                    if(op==JFileChooser.APPROVE_OPTION){
                        File f=fc.getSelectedFile();
                        if(fc.getFileFilter()==fffDME&&!f.getName().toLowerCase().endsWith(".dme"))
                            f=new File(f.getAbsolutePath()+".DME");
                        if(fc.getFileFilter()==fffTXT&&!f.getName().toLowerCase().endsWith(".txt"))
                            f=new File(f.getAbsolutePath()+".txt");
                        if(f.getName().toLowerCase().endsWith(".dme")){
                            if(str.equals("Export all"))
                                ExportUtil.exportAllToDME(table, f);
                            else ExportUtil.exportSeletedToDME(table, f);
                        }
                        else{
                            if(str.equals("Export all"))
                                ExportUtil.exportAllToTXT(table, f);
                            else ExportUtil.exportSeletedToTXT(table, f);
                        }
                    }
                }
                //</diepesc>
